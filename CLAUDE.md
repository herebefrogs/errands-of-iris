 ...
 | `src/js/inputs/keyboard.js`, `inputs/pointer.js`, `inputs/gamepad.js` | Raw input capture only (see Game engine below). `pointer.js` is a **floating D-pad**: an anchor at the contact point, steering vector `(finger − anchor)` per axis ramping 0→±1 over `RAMP` (55px), the anchor *trailing* the finger to stay within `RAMP` (so the pad follows the thumb), a per-axis `DEAD` (8px) zero band that also snaps near-cardinal drags to pure cardinals. Reversal only flips an axis once the finger crosses back through the trailed anchor. Exports `pointerDirection()` (the [-1,1]² vector, `game.js` normalises it — only the angle matters) and `pointerPad()` (anchor/finger/RAMP/DEAD for the overlay drawn in `game.js`, `DEBUG_POINTER` for the full breakdown). `gamepad.js` (Xbox / W3C "standard" mapping, vendored as-is from the gamejam-boilerplate template) exports one `gamepadPollData()` — connect/disconnect events proved unreliable cross-browser, so it just polls `navigator.getGamepads()` every call and returns the full standard mapping (`leftX/Y`, `rightX/Y`, `buttonA/B/X/Y`, bumpers, analog triggers, `back`/`start`, stick-click buttons, `dpadUp/Down/Left/Right`) or `undefined`, no cached state, no press/release events of its own. `game.js` only uses a subset so far (left/right sticks, A/B, D-pad up/down) and does its own edge-detection on top of the raw poll (`gamepadYArmed`/`gamepadAWas`/`gamepadBWas`/`gamepadDpadUpWas`/`DownWas` in `processInputs()`) for one-shot menu-nav/select/back, tracked every frame regardless of screen so a held-over button/stick can't fire the moment a new menu screen appears; the right stick only steers/navigates once the left one reads as fully unused (both axes under its own deadzone) - left has precedence, right is a fallback. |
